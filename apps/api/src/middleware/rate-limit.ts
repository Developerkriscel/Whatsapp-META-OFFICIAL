...
export const PLAN_MESSAGE_LIMITS: Record<string, number> = {
  STARTER: 5000,
  GROWTH: 25000,
  BUSINESS: 100000,
 ...
 */
export async function rateLimitMiddleware(app: FastifyInstance): Promise<void> {
  ...
  await app.register(import('@fastify/rate-limit'), {
    ...
    keyGenerator: (request: FastifyRequest) => {
      ...
      const req = request as FastifyRequest & { authUser?: { id: string } };
      if (req.authUser?.id) {
 ...
 */
export function getMessageRateLimit(planTier: string | undefined): {
  maxRequests: number;
  windowMs: number;
} {
 ...
 */
export function getMonthlyMessageLimit(planTier: string | undefined): number {
...
export class WhatsAppSendRateLimiter {
   ...
   */
  private getPlanAllowance(planTier: string): number {
  ...
  async canSend(
    tenantId: string,
    planTier: string
  ): Promise<{ allowed: boolean; waitMs?: number; remaining?: number }> {
 ...
 */
export function createTenantRateLimiter(planTier: string): TokenBucketRateLimiter {
...
const PLAN_RATE_LIMITERS: Record<string, TokenBucketRateLimiter> = {};

for (const [tier, limit] of Object.entries(RATE_LIMITS)) {
  if (tier !== 'messages_send') {
    PLAN_RATE_LIMITERS[tier] = new TokenBucketRateLimiter(
      limit.maxRequests,
      limit.maxRequests / (limit.windowMs / 1000),
      limit.windowMs
    );
 ...
 */
export function getRateLimiter(type: keyof typeof RATE_LIMITS): TokenBucketRateLimiter {
