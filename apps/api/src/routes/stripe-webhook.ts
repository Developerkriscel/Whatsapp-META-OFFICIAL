      ...
      try {
        switch (event.type) {
          ...
          case 'customer.subscription.updated':
            await handleSubscriptionUpsert(app, event.data.object as Stripe.Subscription);
            break;
          ...
          case 'customer.updated':
            await handleCustomerUpdated(app, event.data.object as Stripe.Customer);
            break;
...
async function handleSubscriptionDeleted(app: FastifyInstance, subscription: Stripe.Subscription) {
  const tenantId = subscription.metadata?.tenantId;
...
async function handleInvoicePaymentFailed(app: FastifyInstance, invoice: Stripe.Invoice) {
  const customerId = typeof invoice.customer === 'string'
    ? invoice.customer
    : invoice.customer?.id;
...
async function handleCustomerUpdated(app: FastifyInstance, customer: Stripe.Customer) {
