...
export async function registerSuperadminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/whatsapp-health', async (request, reply) => {
    ...
    const phonesByTenant = new Map<string, typeof phones>();
    ...
    const tally = <T extends { tenantId: string; status: string; _count: { _all: number } }>(rows: T[]) => {
    ...
    const templatesByTenant = tally(templateCounts as any);
    const messagesByTenant = tally(messageCounts as any);
    ...
    const rows = tenants.map(t => {
      ...
      const stage =
        !hasToken || !cred?.wabaId ? 'NOT_CONNECTED'
      ...
      const issues: string[] = [];
      if (cred?.lastError) issues.push(`Credential error: ${cred.lastError}`);
      if (livePhones.some(p => p.qualityScore === 'RED')) issues.push('A number is rated RED by Meta');
      if (livePhones.some(p => p.nameStatus === 'DECLINED')) issues.push('A display name was declined');
      if ((tpl.REJECTED ?? 0) > 0) issues.push(`${tpl.REJECTED} template(s) rejected`);
  ...
  app.get('/whatsapp-accounts', async (request, reply) => {
    ...
    try {
      ...
      const tenantByWaba = new Map<string, { id: string; name: string }>();
      for (const k of known) if (k.wabaId && k.tenant) tenantByWaba.set(k.wabaId, k.tenant);
  ...
  app.get('/credits/export', async (request, reply) => {
    ...
    const csvRows = transactions.map((tx: any) =>
      `"${new Date(tx.createdAt).toISOString()}","${tx.credit?.tenant?.name || 'N/A'}","${tx.type}","${tx.amount}","${tx.balanceAfter || ''}","${(tx.description || '').replace(/"/g, '""')}"`
    ).join('\n');
