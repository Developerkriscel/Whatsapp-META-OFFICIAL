...
const FLOW_TEMPLATES: Record<string, { name: string; description: string; category: string; flowData: any }> = {
  welcome: {
...
const FLOW_STEP_TYPES = ['trigger', 'message', 'condition', 'delay', 'action', 'ai_reply', 'end'] as const;
...
export async function registerAutomationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/automation/executions', async (request, reply) => {
    ...
    const { page = '1', limit = '20', flowId } = request.query as { page?: string; limit?: string; flowId?: string };
  ...
  app.post('/automation/flows/suggest', { preHandler: [app.requirePermission('flows', 'create')] }, async (request, reply) => {
    ...
    let parsed: { steps?: any[]; variables?: any[] };
    try {
    ...
    const valid =
      ...
      steps.every((s) =>
        [s.next, s.truePath, s.falsePath].every((ref) => ref == null || ids.has(ref))
      );
 ...
 */
export async function triggerFlowForConversation(
  app: FastifyInstance,
  tenantId: string,
...
async function executeFlowStep(
  ...
  const flowData = flow.flowData as { steps: any[]; variables: any[] };
  const steps = flowData?.steps || [];

  const currentStep = steps.find((s: any) => s.id === execution.currentStep) ||
                      steps.find((s: any) => s.type === 'trigger'); // start from first trigger
