...
function parseSignedRequest(signedRequest: string, appSecret: string): SignedRequestPayload {
  ...
  const payload: SignedRequestPayload = JSON.parse(base64UrlDecode(encodedPayload).toString('utf8'));
...
export async function registerMetaComplianceRoutes(app: FastifyInstance): Promise<void> {
  /**
