...
export async function registerSuperadminCreditRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/credit-rates', async (request, reply) => {
    ...
    const belowCost = withMargin.filter(
      (r) => (r.marketing.cost > 0 && r.marketing.marginCredits < 0) ||
  ...
  app.get('/credit-rates/live', async (request, reply) => {
    ...
    const targets: Array<{ wabaId: string; tenantId: string }> = [];
    for (const ph of phones) {
      if (!ph.wabaId || seen.has(ph.wabaId)) continue;
      seen.add(ph.wabaId);
      targets.push({ wabaId: ph.wabaId, tenantId: ph.tenantId });
    }

    type Agg = { country: string; category: string; volume: number; cost: number; currency: string };
    ...
    for (const t of targets) {
      ...
      try {
        ...
        const url =
          `https://graph.facebook.com/v21.0/${t.wabaId}?fields=pricing_analytics` +
        ...
        for (const dp of body?.pricing_analytics?.data?.[0]?.data_points ?? []) {
          if (!dp.country || !dp.pricing_category) continue;
    ...
    const rows = [...agg.values()]
      ...
      .map((a) => {
        ...
        const sell =
          a.category === 'MARKETING' ? cfg?.marketingCredits
  ...
  app.post('/tenants/:tenantId/credits', async (request, reply) => {
    ...
    const result = await addCredits(
      ...
      body.type,
      undefined,
      body.description || `${body.type} credits`,
    );
  ...
  app.post('/tenants/:tenantId/credits/deduct', async (request, reply) => {
    ...
    const result = await deductCredits(
      ...
      'ADMIN_DEDUCTION',
      body.description || 'Manual deduction by admin',
    );
