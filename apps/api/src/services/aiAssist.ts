...
export function checkTemplateContent(input: TemplateCheckInput): TemplateCheckResult {
  const issues: TemplateCheckIssue[] = [];
  const body = input.bodyText ?? '';
  const trimmedBody = body.trim();

  const push = (severity: TemplateCheckIssue['severity'], code: string, message: string, field: TemplateCheckIssue['field']) =>
...
// ============================================
// Optional Mistral-powered suggestions
// ============================================
...
function buildPrompt(input: AISuggestionInput): { system: string; user: string } {
  switch (input.module) {
    case 'template': {
      ...
      return {
        system:
          'You rewrite WhatsApp Business message templates so they comply with Meta\'s template policy. ' +
    ...
    case 'campaign': {
      ...
      return {
        system:
          'You write concise, effective WhatsApp broadcast campaign messages. Keep it under 300 characters, friendly, and specific. ' +
    ...
    case 'segment': {
      ...
      return {
        system:
          `You build audience segment filters for a WhatsApp CRM. Allowed fields: ${(fields || []).join(', ')}. ` +
    ...
    case 'flow': {
      ...
      return {
        system:
          `You design WhatsApp chatbot conversation flows. Allowed step types: ${(stepTypes || []).join(', ')}. ` +
