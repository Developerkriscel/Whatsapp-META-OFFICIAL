...
export async function checkPlanLimits(
  ...
  const planTier = (tenant.plan?.tier as PlanTier) || PlanTier.STARTER;
  ...
  if (!limits.features.teamManagement) blockedFeatures.push('Team Management');
  if (!limits.features.customBranding) blockedFeatures.push('Custom Branding');
  if (!limits.features.apiAccess) blockedFeatures.push('API Access');
  if (!limits.features.whiteLabel) blockedFeatures.push('White Label');
...
export async function checkFeatureAccess(
  prisma: PrismaClient,
  tenantId: string,
  feature: 'teamManagement' | 'customBranding' | 'apiAccess' | 'whiteLabel'
): Promise<boolean> {
  const tenant = await prisma.tenant.findUnique({
    where: { id: tenantId },
    include: { plan: true },
