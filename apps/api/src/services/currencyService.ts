...
export const CURRENCIES: Record<string, {
  ...
  countries: string[]; // Countries billed in this currency
}> = {
  USD: {
    code: 'USD',
    name: 'US Dollar',
...
export const DISPLAY_CURRENCIES: Record<string, {
  code: string;
  name: string;
  symbol: string;
  decimalPlaces: number;
 ...
 */
export function getMetaBillingCurrency(countryCode: string): string {
 ...
 */
export function convertMessageCost(
  credits: number,
 ...
 */
export function getDisplayCurrencies(): Array<{
  code: string;
  name: string;
  symbol: string;
}> {
 ...
 */
export function getMessageCostInLocalCurrency(
  credits: number,
  countryCode: string
): {
  credits: number;
