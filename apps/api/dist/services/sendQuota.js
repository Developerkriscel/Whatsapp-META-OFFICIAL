 ...
 */
export async function getTierUsage(prisma, phoneNumberId) {
 ...
 */
export async function checkTierCapacity(prisma, phoneNumberId, newCustomers) {
    ...
    return {
        withinTier: false,
        usage,
        message: `This would message ${newCustomers} people, but Meta's ${tierName} limit allows ${usage.limit} ` +
...
export async function reserveSendSlot(prisma, phoneNumberId) {
    ...
    const rows = await prisma.$queryRawUnsafe(`UPDATE phone_numbers
      ...
      RETURNING "todaySentCount", "dailySentLimit"`, phoneNumberId);
    if (rows.length > 0) {
        return {
            allowed: true,
 ...
 */
export async function releaseSendSlot(prisma, phoneNumberId) {
    await prisma
        .$executeRawUnsafe(`UPDATE phone_numbers
          SET "todaySentCount" = GREATEST("todaySentCount" - 1, 0)
        WHERE id = $1`, phoneNumberId)
        .catch(() => {
        // Best effort — a failed release must never mask the send error that
        // triggered it.
    });
}
//# sourceMappingURL=sendQuota.js.map