...
export async function registerInsightsRoutes(app) {
    ...
    app.get('/insights/overview', async (request, reply) => {
        ...
        // Contacts stats
        totalContacts, optedInContacts, optedOutContacts, 
        // Campaign stats
        totalCampaigns, campaignsThisMonth, 
        // Conversation stats
        openConversations, avgResponseTime, 
        // WhatsApp health
    ...
    app.get('/insights/campaigns', async (request, reply) => {
        ...
        const deliveryRates = avgOpenRate.map(c => c.totalSent && c.totalRecipients ? (c.totalSent / c.totalRecipients) * 100 : 0);
    ...
    app.get('/insights/inbox', async (request, reply) => {
        ...
        const weekAgo = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);
        // Conversation stats
        const [totalConversations, openConversations, closedToday, openedThisWeek,] = await Promise.all([
    ...
    app.get('/insights/finance', async (request, reply) => {
        ...
        // Get usage stats
        const [totalContacts, totalMessages, totalCampaigns, totalConversations,] = await Promise.all([
            app.prisma.contact.count({ where: { tenantId, isActive: true } }),
            app.prisma.message.count({ where: { tenantId, direction: 'OUTGOING' } }),
            app.prisma.campaign.count({ where: { tenantId } }),
            app.prisma.conversation.count({ where: { tenantId } }),
