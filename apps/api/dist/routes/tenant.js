...
export async function registerTenantRoutes(app) {
    ...
    app.get('/conversations/stats', async (request, reply) => {
        ...
        const stats = {
            ...
            pendingConversations: statusCounts.find(s => s.status === 'PENDING')?._count || 0,
            messages: {
                total: messageStats.reduce((acc, s) => acc + s._count, 0),
                inbound: messageStats.filter(s => s.direction === 'INCOMING' || s.direction === 'INBOUND').reduce((acc, s) => acc + s._count, 0),
    ...
    app.post('/campaigns/:campaignId/send', async (request, reply) => {
        ...
        const { getRateCredits } = await import('../services/creditService.js');
    ...
    app.get('/analytics/overview', async (request, reply) => {
        ...
        const { period = '30d', start, end } = request.query;
        const { startDate, endDate } = getDateRange(period, start, end);
        const [totalMessages, sentMessages, deliveredMessages, readMessages, failedMessages, activeContacts, conversations,] = await Promise.all([
            app.prisma.message.count({
        ...
        conversations.forEach(c => {
            ...
            if (c.status === 'OPEN')
                conversationStats.open = c._count;
            else if (c.status === 'CLOSED')
                conversationStats.closed = c._count;
            else if (c.status === 'PENDING_AGENT')
                conversationStats.pending = c._count;
        });
        // Calculate rates
    ...
    app.get('/analytics/export', async (request, reply) => {
        ...
        const { period = '30d', format = 'csv', start, end } = request.query;
        const { startDate, endDate } = getDateRange(period, start, end);
    ...
    for (let i = 0; i < contactIds.length; i += BATCH_SIZE) {
        ...
        const phonePromises = contacts.map(async (contact) => {
            try {
                ...
                await deductCredits(app.prisma, tenantId, costCredits, message.id, 'CAMPAIGN', `Campaign: ${campaign.name} to ${contact.phone}`);
                sent++;
            }
            catch (err) {
