...
export async function registerSystemRoutes(app) {
    ...
    app.get('/api/health/detailed', async (request, reply) => {
        ...
        if (recentRequests.length > 0) {
            status.metrics = {
                ...
                averageResponseTime: Math.round(recentRequests.reduce((sum, r) => sum + r.duration, 0) / recentRequests.length),
                errorRate: Math.round((recentRequests.filter(r => r.status >= 400).length / recentRequests.length) * 100),
            };
        }
     ...
     */
    app.get('/debug/webhook-logs', async (request, reply) => {
     ...
     */
    app.get('/debug/requests', async (request, reply) => {
