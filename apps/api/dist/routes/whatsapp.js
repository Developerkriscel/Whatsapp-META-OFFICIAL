...
export async function registerWhatsAppRoutes(app) {
    ...
    app.get('/whatsapp/embedded-signup/config', async (request, reply) => {
        ...
        const business = {};
        if (tenant?.name?.trim())
            business.name = tenant.name.trim();
        if (owner?.email)
    ...
    app.post('/whatsapp/phone-numbers/:id/refresh-quality', { preHandler: [app.requirePermission('phone_numbers', 'update')] }, async (request, reply) => {
        ...
        let nameStatus = phone.nameStatus;
        let messagingLimitTier = phone.messagingLimitTier;
        // 2. Try fetching real Meta Graph API quality rating & line status
        if (token && metaPhoneId) {
            try {
                const metaRes = await fetch(`https://graph.facebook.com/v19.0/${metaPhoneId}?fields=display_phone_number,quality_rating,name_status,status,messaging_limit_tier&access_token=${token}`);
                if (metaRes.ok) {
                    ...
                    if (metaData.name_status)
                        nameStatus = String(metaData.name_status).toUpperCase();
                    if (metaData.messaging_limit_tier)
                        messagingLimitTier = String(metaData.messaging_limit_tier).toUpperCase();
                }
            }
            catch (e) {
    ...
    app.get('/whatsapp/business-verification', async (request, reply) => {
        ...
        let officialBusinessAccount = null;
    ...
    app.patch('/whatsapp/webhook/settings', { preHandler: [app.requirePermission('settings', 'update')] }, async (request, reply) => {
        ...
        return reply.status(409).send({
            ...
            error: {
                ...
                message: 'Webhook subscriptions are configured once for the whole platform and cannot be changed per workspace. ' +
    ...
    app.post('/whatsapp/webhook/test', { preHandler: [app.requirePermission('settings', 'update')] }, async (request, reply) => {
        ...
        if (!verifyToken) {
            ...
            try {
                const res = await fetch(`${webhookUrl}?hub.mode=subscribe&hub.verify_token=${encodeURIComponent(verifyToken)}&hub.challenge=${challenge}`, { signal: AbortSignal.timeout(10_000) });
