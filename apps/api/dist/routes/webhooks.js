...
// ============================================
export async function processWhatsAppWebhook(app, body) {
    try {
...
async function processTemplateStatusUpdate(app, value) {
    ...
    if (updated.count > 0) {
        ...
        if (template) {
            ...
            await logWebhookEvent(app, template.tenantId, null, 'message_template_status_update', metaTemplateId, null, { event: value.event, status, name: template.name }, 'COMPLETED').catch(() => {
                // Logging must never break status processing.
            });
        }
    }
}
async function processIncomingMessage(app, message, phoneNumber, tenantId) {
    ...
    const newMessage = await app.prisma.message.create({
        data: {
            ...
            direction: 'INCOMING',
            type: parsed.type.toUpperCase() || 'TEXT',
            body: parsed.body,
...
async function processStatusUpdate(app, status, tenantId) {
 ...
 */
async function logWebhookEvent(app, tenantId, phoneNumberId, event, messageId, from, rawPayload, webhookStatus) {
    try {
