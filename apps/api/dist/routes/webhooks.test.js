...
function makeMockApp(phoneNumberRecord) {
...
function makeIncomingMessagePayload(phoneNumberId) {
    return {
        entry: [
            {
                changes: [
...
describe('processWhatsAppWebhook - tenant routing', () => {
    it('routes an incoming message to the tenant that owns the matching phone number', async () => {
        ...
        expect(app.prisma.message.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('routes each change to the correct tenant when multiple phone numbers are involved', async () => {
        ...
        const app = makeMockApp(null);
        app.prisma.phoneNumber.findUnique = vi.fn(({ where }) => Promise.resolve(lookups[where.metaPhoneId] ?? null));
        const payload = {
            entry: [
                {
                    changes: [
        ...
        const tenantIdsUsed = app.prisma.contact.findFirst.mock.calls.map((call) => call[0].where.tenantId);
