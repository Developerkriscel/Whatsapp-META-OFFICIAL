 ...
 */
import { z } from 'zod';
import { creditsToUsd, addCredits, getTenantCreditInfo, META_RATES, COUNTRY_NAMES, } from '../services/creditService.js';
import { convertMessageCost, getDisplayCurrencies, getCurrencyForCountry, formatCurrency, DISPLAY_CURRENCIES, } from '../services/currencyService.js';
export async function registerCreditRoutes(app) {
    // ============================================
    ...
    app.get('/credits/rates', async (request, reply) => {
        ...
        const filtered = query.country
            ? rates.filter(r => r.countryCode.toLowerCase().includes(query.country.toLowerCase()) ||
    ...
    app.post('/credits/purchase', { preHandler: [app.requirePermission('billing', 'update')] }, async (request, reply) => {
        ...
        const result = await addCredits(app.prisma, request.authUser.tenantId, creditsToAdd, 'PURCHASE', body.packId || 'custom', description);
        return {
            success: true,
            data: {
    ...
    app.post('/credits/bonus', { preHandler: [app.requirePermission('billing', 'update')] }, async (request, reply) => {
        ...
        const result = await addCredits(app.prisma, request.authUser.tenantId, body.amount, 'BONUS', undefined, body.description || 'Bonus credits');
        return {
            success: true,
            data: {
